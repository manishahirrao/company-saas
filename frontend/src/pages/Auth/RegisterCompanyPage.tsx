...
const CompanyRegisterPage: React.FC = () => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.companyEmail.trim()) {
      ...
      newErrors.companyEmail = 'Please use a company email address, not a personal email';
    }
    ...
    if (!formData.linkedinUrl.trim()) {
      newErrors.linkedinUrl = 'LinkedIn page URL is required';
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (result.user) {
        ...
        if (formData.linkedinUrl) companyData['linkedin_url'] = formData.linkedinUrl;
        if (formData.industry) companyData['industry'] = formData.industry;
        if (formData.companySize) companyData['size'] = formData.companySize;
        if (formData.location) companyData['location'] = formData.location;
      ...
      // On successful registration, redirect to verify page
